...
// Map grossière allouée en mémoire au démarrage (downsamplée depuis la carte
// SD)
...
void setCoarseObstacle(int cx, int cy, bool isObstacle) {
  if (cx >= 0 && cx < coarseW && cy >= 0 && cy < coarseH &&
      coarseMap != nullptr) {
    if (isObstacle)
      coarseMap[cy * coarseW + cx] = 1;
  }
}
...
bool computeAStarPath(float startX_m, float startY_m, float endX_m,
                      float endY_m) {
  if (!coarseMap)
    return false;
  ...
  while (openCount > 0 && iterations < MAX_ITER) {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      if (closedSet[nIdx] || coarseMap[nIdx] == 1)
        continue;

      // Vérifier les diagonales (éviter de couper les coins)
      if (i >= 4) { // directions diagonales
        if (coarseMap[cy * coarseW + nx] == 1 ||
            coarseMap[ny * coarseW + cx] == 1)
          continue;
      }
  ...
  if (found) {
    ...
    // Fallback : se diriger vers la cellule la plus proche de la destination
    // qu'on a visitée
    int bestDist = 999999;
