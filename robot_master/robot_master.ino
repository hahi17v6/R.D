...
ArUcoMarker balises[] = {
    ...
    {15, 7.45, 0.86, 0.0},   // Chambre F-041
    {16, 7.39, 1.31, 0.0},   // Chambre F-042
    {13, 8.16, 14.28, 0.0},  // Chambre F-034
    {14, 8.08, 17.73, 0.0},  // Chambre F-034 (porte 2)
    {12, 7.97, 21.30, 0.0},  // Chambre F-033
    {11, 18.39, 36.88, 0.0}, // Chambre F-032
    {10, 15.34, 51.49, 0.0}, // Chambre F-030
    {8, 13.22, 54.30, 0.0},  // F-029 — Cuisine (entrée principale)
    {9, 3.45, 54.25, 0.0},   // F-029 — Cuisine (entrée 2)
    {7, 1.34, 41.77, 0.0},   // Chambre F-028
    {5, 1.48, 48.54, 0.0},   // Chambre F-027 (1ère porte)
    {6, 15.41, 44.43, 0.0},  // Chambre F-027 (2ème porte)
    {4, 1.48, 51.21, 0.0},   // Chambre F-026
    {2, 1.48, 48.54, 0.0},   // Chambre F-025 (1ère porte)
    {3, 15.07, 48.71, 0.0},  // Chambre F-025 (2ème porte)
    {1, 1.44, 51.32, 0.0},   // Chambre F-024

    // === INTERSECTIONS & REPÈRES ===
    {17, 7.78, 6.26, 0.0},   // Intersection 1
    {18, 8.10, 11.16, 0.0},  // Avant l'Intersection 1
    {19, 7.89, 20.45, 0.0},  // Local technique
    {20, 8.69, 34.85, 0.0},  // Intersection 2
    {21, 10.86, 35.64, 0.0}, // Intersection 3
    {22, 16.76, 35.79, 0.0}, // Intersection 4
    {23, 10.99, 37.02, 0.0}, // Intersection 5
    {24, 15.20, 38.33, 0.0}, // Intersection 6
    {25, 15.26, 53.16, 0.0}, // Avant l'Intersection 2
    {26, 15.10, 35.89, 0.0}, // Début de couloir
    {27, 1.29, 55.49, 0.0},  // Intersection 7
    {28, 1.34, 39.31, 0.0},  // Porte random 1
    {29, 1.46, 5.90, 0.0},   // Porte random 2
};
...
const int ARUCO_KITCHEN = 8; // ID ArUco de la cuisine (F-029 entrée principale)
...
void handleArUcoDetection(int arucoId) {
  ...
  for (int i = 0; i < NB_BALISES; i++) {
    if (balises[i].id == arucoId) {
      ...
      Serial.printf("[ARUCO] Balise #%d détectée → Position recalibrée: (%.2f, "
                    "%.2f) → (%.2f, %.2f)\n",
  ...
  if (!knownMarker) {
    // Info si le marqueur n'est pas dans le tableau de recalibration (ex: sert
    // juste de trigger d'arrivée)
    Serial.printf(
        "[ARUCO] Balise #%d détectée (non utilisée pour recalibration XY)\n",
        arucoId);
  }
  ...
  // On utilise ARUCO_HOME ou ARUCO_KITCHEN comme base de retour (les deux
  // marchent si la base est la cuisine)
...
void sendToArduino(String cmd) {
  if (cmd == "FWD")
    sendToArduinoBinary(1);
  else if (cmd == "BACK")
    sendToArduinoBinary(2);
  else if (cmd == "LEFT")
    sendToArduinoBinary(3);
  else if (cmd == "RIGHT")
    sendToArduinoBinary(4);
  else if (cmd == "STOP")
    sendToArduinoBinary(5);
  else if (cmd.startsWith("UNLOCK:"))
    sendToArduinoBinary(6, cmd.substring(7).toInt());
  else if (cmd == "SERVO:OPEN")
    sendToArduinoBinary(7);
  else if (cmd == "SERVO:CLOSE")
    sendToArduinoBinary(8);
  else if (cmd.startsWith("LCD:")) {
    String txt = cmd.substring(4);
    if (txt == "Code OK!")
      sendToArduinoBinary(9, 1);
    else if (txt == "Code faux!")
      sendToArduinoBinary(9, 2);
    else if (txt == "Entrez le code")
      sendToArduinoBinary(9, 3);
    else if (txt.startsWith("Code: "))
      sendToArduinoBinary(9, 4);
  }
}
...
void processArduinoMessageBinary(SensorPacket &sp) {
  ...
  while (robotAngle >= 360.0)
    robotAngle -= 360.0;
  while (robotAngle < 0.0)
    robotAngle += 360.0;
...
void navigationStep() {
  ...
  if (distFront < 20)
    return; // Sécurité locale gérée par Uno
...
void startMission(String orderId, String room) {
  if (missionState != MISSION_IDLE) {
    ...
    if (queueCount < MAX_QUEUE) {
      ...
      Serial.printf(
          "[QUEUE] Commande %s ajoutée en file d'attente (position: %d)\n",
      ...
      doc["msg"] =
          "File d'attente pleine (max " + String(MAX_QUEUE) + " commandes)";
      String json;
      serializeJson(doc, json);
      webSocket.broadcastTXT(json);
    }
  ...
  // Si la chambre n'est pas dans le registre, on essaie de convertir le nom en
  // ID (ex "101" -> 101)
  if (targetArUco == -1)
    targetArUco = room.toInt();

  missionState = GOING_TO_KITCHEN;
  missionStartTime = millis(); // Démarrer le timer de timeout

  Serial.printf(
      "[MISSION] Démarrage : %s → Chambre %s (ArUco Cible: %d, code: %s)\n",
...
void handleKeypad() {
  ...
  if (missionState != WAITING_DELIVERY)
    return;
  ...
  if (key == '#') {
    ...
    if (keypadBuffer.length() < 6) {
      ...
      String stars = "";
      for (int i = 0; i < (int)keypadBuffer.length(); i++)
        stars += "*";
...
void returnedHome() {
  ...
  if (batteryLevel <= 20) {
    ...
    Serial.println("[MISSION] De retour à la cuisine — BATTERIE FAIBLE, robot "
                   "en veille/charge");
  } else {
    missionState = MISSION_IDLE;
    doc["status"] = "Disponible";
    Serial.println("[MISSION] De retour à la cuisine — Robot disponible pour "
...
void handleFileRead() {
  ...
  // Endpoint : authentification côté serveur (PINs + staff secrets ici, pas
  // dans le HTML)
  if (path == "/auth") {
    if (!server.hasArg("role")) {
      server.send(400, "application/json",
                  "{\"ok\":false,\"msg\":\"Missing role\"}");
      return;
    }
    ...
    if (role == "staff") {
      ...
      if ((user == "admin" && pass == "robot2024") ||
        ...
        server.send(401, "application/json",
                    "{\"ok\":false,\"msg\":\"Identifiants incorrects\"}");
      }
    } else if (role == "patient") {
      String room = server.arg("room");
      String pin = server.arg("pin");
      // PINs résidents (sécurisés côté serveur)
      // Format : les 4 derniers chiffres du numéro de chambre, ou PIN
      // personnalisé
      struct RoomPin {
        const char *room;
        const char *pin;
      ...
      if (valid) {
        ...
        server.send(401, "application/json",
                    "{\"ok\":false,\"msg\":\"Chambre ou PIN incorrect\"}");
      }
    } else {
      server.send(400, "application/json",
                  "{\"ok\":false,\"msg\":\"Invalid role\"}");
    }
    return;
  }
...
void webSocketEvent(uint8_t num, WStype_t type, uint8_t *payload,
  ...
  case WStype_CONNECTED: {
    ...
    if (missionState == NEEDS_CHARGE)
      doc["status"] = "En charge (Batterie faible)";
    else if (missionState == MISSION_IDLE)
      doc["status"] = "Disponible";
    else
      doc["status"] = "En mission";
