...
struct CommandPacket {
  uint8_t header; // 0xAA
  uint8_t action; // 1=FWD, 2=BACK, 3=LEFT, 4=RIGHT, 5=STOP, 6=UNLOCK,
                  // 7=SERVO_OPEN, 8=SERVO_CLOSE, 9=LCD_MSG
...
struct SensorPacket {
  ...
  int8_t deltaDist_cm; // Estimation de la distance parcourue en cm depuis le
                       // dernier envoi
};
#pragma pack(pop)
...
void mettreAJourRampe() {
  if (millis() - dernierUpdate < DELAI_RAMPE)
    return;
  ...
  if (vitesseA > 0 && dirA != nouvelleDirA) {
    ...
    if (dirA != nouvelleDirA)
      dirA = nouvelleDirA;
    if (vitesseA < cibleA)
      vitesseA = min(cibleA, vitesseA + PAS_RAMPE);
    if (vitesseA > cibleA)
      vitesseA = max(cibleA, vitesseA - PAS_RAMPE);
  }
  ...
  if (vitesseB > 0 && dirB != nouvelleDirB) {
    ...
    if (dirB != nouvelleDirB)
      dirB = nouvelleDirB;
    if (vitesseB < cibleB)
      vitesseB = min(cibleB, vitesseB + PAS_RAMPE);
    if (vitesseB > cibleB)
      vitesseB = max(cibleB, vitesseB - PAS_RAMPE);
  }
...
void decisionLocale() {
  if (currentMode == MODE_DELIVERY || currentMode == MODE_IDLE)
    return;
  ...
  if (currentMode == MODE_FORWARD) {
    if (abs(diff) > SEUIL_INTERSECTION) {
      ...
      // distLeft > distRight => plus de place à gauche => se décaler à gauche
      // (donc tourner gauche)
      cmdVirerGauche();
    } else {
      // distLeft < distRight => plus de place à droite => se décaler à droite
      // (donc tourner droite)
      cmdVirerDroite();
    }
  }
...
void envoyerEtat() {
  ...
  // Si le robot avance à VITESSE_BASE (180), on suppose ~0.5 m/s maximum (pour
  // 255)
  ...
  accum_dist_cm +=
      (approxSpeed_ms * dt) * 100.0; // Distance parcourue en cm pendant dt
  sp.deltaDist_cm = (int8_t)accum_dist_cm;
  accum_dist_cm -=
      sp.deltaDist_cm; // On garde le reste décimal pour le prochain cycle

  Serial.write((uint8_t *)&sp, sizeof(SensorPacket));
  if (pendingEvent != 0)
    pendingEvent = 0; // Consumé
}

void afficherPret() {
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print(F("Robot pret"));
  lcd.setCursor(0, 1);
...
void loop() {
  ...
  if (delayState != DELAY_NONE) {
    ...
    case AVOID_WAIT_5S:
      if (elapsed >= 5000) {
        ...
        if (distFront >= SEUIL_OBSTACLE_AVANT) {
          ...
          lcd.print(F("Esquive..."));
          lcd.setCursor(0, 1);
          lcd.print(avoidGoRight ? F("-> Droite") : F("<- Gauche"));
          if (avoidGoRight)
            cmdPivotDroite();
          else
            cmdPivotGauche();
    ...
    case AVOID_PIVOT_AWAY: {
      ...
      if (angleTurned > 180.0)
        angleTurned = 360.0 - angleTurned;
  ...
  if (currentMode != MODE_IDLE && currentMode != MODE_DELIVERY) {
    if (millis() - dernierPaquetRecu > WATCHDOG_TIMEOUT_MS) {
      ...
      lcd.clear();
      lcd.setCursor(0, 0);
      lcd.print(F("PERTE ESP32!"));
      lcd.setCursor(0, 1);
      lcd.print(F("Arret securite"));
    }
  }
